...
export function BackendHealthPanel() {
  ...
  const stale = hbAge != null && hbAge > 60;
  const degraded = !stale && hbAge != null && hbAge > 15;
  const ingest = stale ? "STALE" : degraded ? "DEGRADED" : hbAge == null ? "UNKNOWN" : "OK";
  const ingestTone = stale ? "red" : degraded ? "orange" : hbAge == null ? "gray" : "green";
    ...
    <Panel
      ...
      right={
        <span className="flex items-center gap-2">
          <Badge value={`INGEST: ${ingest}`} tone={ingestTone as any} />
          {stale && <Badge value="BACKEND STALE" tone="red" />}
        </span>
      }
    >
      <div className="grid grid-cols-3 gap-x-4 gap-y-0.5">
        <KV k="Backend Heartbeat" v={<span className={ageTone(hbAge)}>{hbAge == null ? "—" : `${hbAge}s ago`}</span>} />
        ...
        <KV k="Supabase Channel" v={<b className={rt === "CONNECTED" ? "text-profit" : "text-loss"}>{rt}</b>} />
        <KV k="RDP" v={rdp} />
      ...
      {stale && (
        <div className="mt-2 border-2 border-red-700 bg-red-100 text-red-900 px-2 py-1 text-[10px] uppercase tracking-widest">
          BACKEND STALE — all strategy cards marked stale · "trade ready" suppressed · dashboard remains read-only.
        </div>
      )}
      {degraded && (
        <div className="mt-2 border border-orange-700 bg-orange-50 text-orange-900 px-2 py-1 text-[10px] uppercase tracking-widest">
          Remote dashboard ingest degraded. MT5 backend can still run locally. Dashboard data may lag.
        </div>
      )}
    </Panel>
  );
